...
class DelayedTransportTest extends TestCase
{
    ...
    {
        $guzzleClient = m::mock(\GuzzleHttp\Client::class.'[get]');
        $guzzleClient->shouldReceive('get')
            ->once()
            ->withArgs([
                'https://trk.kissmetrics.com/e?_n=Runned%20test&_p=example%40example.com&_k=test&_t='.time().'&_d=1',
            ]);

        $guzzleClient->shouldReceive('get')
            ->once()
            ->withArgs([
                'https://trk.kissmetrics.com/e?_n=Runned%20test%202&_p=example2%40example.com&_k=test&_t='.time().'&_d=1',
            ]);

        $guzzleClient->shouldReceive('get')
            ->once()
            ->withArgs([
                'https://trk.kissmetrics.com/s?property=value_1&_k=test&_p=example3%40example.com&_t='.time().'&_d=1',
            ]);
        ...
        $this->assertTrue($this->kmTransport->sendLoggedData());
    }
