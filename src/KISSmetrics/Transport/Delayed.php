...
class Delayed implements Transport
{
     ...
     * @var string
     */
    protected $logFilename = 'kissmetrics_query.log';

    public function __construct(
        string $apiEndpoint = 'https://trk.kissmetrics.com',
    ...
    protected function getLogFile(): string
    {
        ...
        // Prevent file_not_found errors since the methods submitData and sendLoggedData don't check if the file exists
        if (!file_exists($logFile)) {
            touch($logFile);
        }

        return $logDir.'/'.$this->logFilename;
    }
    ...
    public function sendLoggedData(): bool
    {
        $groupedQueries = $this->getFormattedQueries();
        ...
        try {
            foreach ($groupedQueries as $queryGroup) {
                foreach ($queryGroup as $queries) {
                    ...
                    $this->guzzleClient->get($this->apiEndpoint.'/'.$queries[0].'?'.$query);
                }
            }
    ...
    protected function getFormattedQueries(): array
    {
